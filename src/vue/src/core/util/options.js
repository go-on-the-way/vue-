...
if (process.env.NODE_ENV !== 'production') {
  strats.el = strats.propsData = function (parent, child, vm, key) {
    ...
    // 而子组件的实现方式就是通过实例化子类完成的，
    // 子类又是通过 Vue.extend 创造出来的，所以我们就能通过对 vm 的判断而得知是否是子组件了。
    if (!vm) {
      warn(
...
function mergeData (to: Object, from: ?Object): Object {
  ...
  // Reflect.ownKeys与Object.keys的区别在于前者可以列举出非枚举的属性和Symbol属性
...
// 最终得到的 data 选项是一个函数，且该函数的执行结果就是最终的数据对象。
...
export function validateComponentName (name: string) {
  // 等价于 /^[a-zA-Z][\w-]*$/ Vue限定组件的名字由普通的字符和中横线(-)组成，且必须以字母开头。
...
export function mergeOptions (
  ...
  // 规范化props,开发者按不同的方式传入props,vue中做规范化处理,转换成相同的格式
